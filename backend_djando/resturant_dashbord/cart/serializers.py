...
class ShopCartSerializer(serializers.ModelSerializer):
    class Meta:
        model = Cart
        fields = ('id', 'item', 'quantity', 'cartid', 'cartno', 'item_cost','cartn')

    item_cost = serializers.ReadOnlyField()
    
    def get_total_cost(self, obj):
       
        total_cost = Cart.objects.all().aggregate(total_cost=Sum(F('quantity') * F('book__price')))['total_cost']
...
class OrdersSerializer(serializers.ModelSerializer):
    ...
    itemsorders = serializers.PrimaryKeyRelatedField(queryset=Cart.objects.all(), many=True, required=False)
